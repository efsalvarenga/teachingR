...
However, the file to be loaded for the next exercises is an MS Excel spreadsheet.
For this, we will need to install and load a new library called `readxl`.
...
```{r}
readxl::read_excel(path = 'https://www.eia.gov/electricity/data/state/annual_generation_state.xls')
```
