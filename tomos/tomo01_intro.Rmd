...
Objetos armazenam dados em diferentes tipos de estrutuas.
...
Essas estruturas são úteis para operações matriciais (matriz de correlação, multiplicacões vetoriais, etc.).
...
Essa estrutura é muito similar a uma planila do MS Excel.

Algumas funções dependem de um determinado tipo de esturura para sua execução, e funções que transformam entre tipos de estruturas diferentes estão disponiveis.
Outros tipos de estruturas estão disponíveis ( [Data types](https://www.statmethods.net/input/datatypes.html)).

### Atribuição de valores a objetos

Valores podem ser atribídos a objetos diretamente pelo usuário ou por meio do resultado de funções (incluindo funções que importam valores de outras fontes como planilhas, banco de dados, etc.).
...
Para obter ajuda com relação a uma função digite o caracter `?` antes do nome da função, ou pressione **F1** com o cursor sobre a função.
Alternativamente, utilize a aba _Help_ do R Studio ou procure na internet.

### Encadeamento de funções

Funções podem ser encadeadas ao invés de avaliadas objeto a objeto, como abaixo.
